...
import org.ruby.client.ui.InstantAutoCompleteTextView;
...
public class CallScreen extends Activity implements DialogInterface.OnClickListener {
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (Receiver.mEngine != null &&
				Receiver.mEngine.ua != null &&
				Receiver.mEngine.ua.audio_app != null) {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case VIDEO_MENU_ITEM:
			...
			try {
				intent = new Intent(this, org.ruby.client.ui.VideoCamera.class);
	...
	void disableKeyguard() {
    	if (mKeyguardManager == null) {
	        ...
	        mKeyguardLock = mKeyguardManager.newKeyguardLock("Ruby");
	        enabled = true;
    	}
		if (enabled) {
				...
				public void run() {
					...
					try {
						if (intent == null || rtp_socket == null) {
							rtp_socket = new RtpSocket(socket = new RubySocket(Receiver.engine(mContext).getLocalVideo()),
					...
					for (;;) {
						...
						if (videopacket.getPayloadLength() > 200) {
							if (intent != null) {
								...
								Intent i = new Intent(mContext, org.ruby.client.ui.VideoCamera.class);
