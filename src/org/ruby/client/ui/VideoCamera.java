...
public class VideoCamera extends CallScreen implements 
	RubyListener, SurfaceHolder.Callback, MediaRecorder.OnErrorListener, MediaPlayer.OnErrorListener, OnClickListener, OnLongClickListener {
    ...
    public void onStart() {
        ...
        videoQualityHigh = PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.ruby.client.ui.Settings.PREF_VQUALITY, org.ruby.client.ui.Settings.DEFAULT_VQUALITY).equals("high");
    ...
    public void onResume() {
        ...
        if (!justplay) {
			...
			try {
				lss = new LocalServerSocket("Ruby");
				receiver.connect(new LocalSocketAddress("Ruby"));
				...
				if (!Ruby.release) e1.printStackTrace();
				super.onResume();
				finish();
				return;
			}
	        ...
	        startVideoRecording();
        } else if (Receiver.engine(mContext).getRemoteVideo() != 0 && PreferenceManager.getDefaultSharedPreferences(this).getString(org.ruby.client.ui.Settings.PREF_SERVER, org.ruby.client.ui.Settings.DEFAULT_SERVER).equals(org.ruby.client.ui.Settings.DEFAULT_SERVER)) {
        	mVideoFrame.setVideoURI(Uri.parse("rtsp://"+Receiver.engine(mContext).getRemoteAddr()+"/"+
        		Receiver.engine(mContext).getRemoteVideo()+"/ruby"));
    ...
    private void startVideoRecording() {
        Log.v(TAG, "startVideoRecording");
            ...
            if (Receiver.listener_video == null) {
                ...
                try {
					if (rtp_socket == null)
						rtp_socket = new RtpSocket(new RubySocket(Receiver.engine(mContext).getLocalVideo()),
