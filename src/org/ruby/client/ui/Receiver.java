...
import org.ruby.media.Bluetooth;
import org.ruby.media.RtpStreamReceiver;
import org.ruby.media.RtpStreamSender;
import org.ruby.client.*;
import org.ruby.client.phone.Call;
import org.ruby.client.phone.Connection;
import org.zoolu.sip.provider.SipProvider;
	...
	public class Receiver extends BroadcastReceiver {
		...
		public static RubyEngine mEngine;
		
		public static Context mContext;
		public static RubyListener listener_video;
		...
		public static synchronized RubyEngine engine(Context context) {
			mContext = context;
			if (mEngine == null) {
				mEngine = new RubyEngine();
				mEngine.StartEngine();
			...
			if (call_state != state) {
				...
				case UserAgent.UA_STATE_INCOMING_CALL:
					...
					if ((pstn_state == null || pstn_state.equals("IDLE")) &&
							PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_AUTO_ON, org.ruby.client.ui.Settings.DEFAULT_AUTO_ON) &&
						...
						if (am.getStreamVolume(AudioManager.STREAM_RING) > 0) {				 
							String sUriSipRingtone = PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.ruby.client.ui.Settings.PREF_SIPRINGTONE,
					...
					if (wl == null) {
						...
						wl = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK |
								PowerManager.ACQUIRE_CAUSES_WAKEUP, "Ruby.onState");
					}
					wl.acquire();
		...
		public static void onText(int type,String text,int mInCallResId,long base) {
			if (mEngine != null && type == REGISTER_NOTIFICATION+mEngine.pref) {
				cache_text = text;
				cache_res = mInCallResId;
			}
			if (type >= REGISTER_NOTIFICATION && mInCallResId == R.drawable.sym_presence_available &&
					!PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.ruby.client.ui.Settings.PREF_REGISTRATION, org.ruby.client.ui.Settings.DEFAULT_REGISTRATION))
				text = null;
	        ...
	        if (text != null) {
				...
				if (type == MISSED_CALL_NOTIFICATION) {
			        	...
			        	if (PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).getBoolean(org.ruby.client.ui.Settings.PREF_NOTIFY, org.ruby.client.ui.Settings.DEFAULT_NOTIFY)) {
		        	...
		        	case AUTO_ANSWER_NOTIFICATION:
						...
						break;
		        	default:
		        		if (type >= REGISTER_NOTIFICATION && mEngine != null && type != REGISTER_NOTIFICATION+mEngine.pref &&
		        				mInCallResId == R.drawable.sym_presence_available)
							notification.contentIntent = PendingIntent.getActivity(mContext, 0,
						            createIntent(ChangeAccount.class), 0);
		        		else
		        			notification.contentIntent = PendingIntent.getActivity(mContext, 0,
		        					createIntent(Ruby.class), 0);
					...
					if (base != 0) {
						...
						if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_POS, org.ruby.client.ui.Settings.DEFAULT_POS))
						...
						if (mEngine != null)
							contentView.setTextViewText(R.id.text1,
									mEngine.user_profiles[type-REGISTER_NOTIFICATION].username+"@"+
									mEngine.user_profiles[type-REGISTER_NOTIFICATION].realm_orig);
	        		} else
	        ...
	        if (type != AUTO_ANSWER_NOTIFICATION)
	        	updateAutoAnswer();
			if (mEngine != null && type >= REGISTER_NOTIFICATION && type != REGISTER_NOTIFICATION+mEngine.pref)
				onText(REGISTER_NOTIFICATION+mEngine.pref,cache_text,cache_res,0);
		}
		
		static void updateAutoAnswer() {
			if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_AUTO_ONDEMAND, org.ruby.client.ui.Settings.DEFAULT_AUTO_ONDEMAND) &&
				Ruby.on(mContext)) {
				if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_AUTO_DEMAND, org.ruby.client.ui.Settings.DEFAULT_AUTO_DEMAND))
					updateAutoAnswer(1);
				else
					updateAutoAnswer(0);
			} else
		...
		public static void pos(boolean enable) {
			...
			pos_gps(false);
			if (enable) {
				if (call_state == UserAgent.UA_STATE_IDLE && Ruby.on(mContext) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_POS, org.ruby.client.ui.Settings.DEFAULT_POS) &&
						PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.ruby.client.ui.Settings.PREF_POSURL, org.ruby.client.ui.Settings.DEFAULT_POSURL).length() > 0) {
		...
		static void enable_wifi(boolean enable) {
			if (!PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_OWNWIFI, org.ruby.client.ui.Settings.DEFAULT_OWNWIFI))
				return;
			if (enable && !PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_WIFI_DISABLED, org.ruby.client.ui.Settings.DEFAULT_WIFI_DISABLED))
        		return;
    		...
    		edit.putBoolean(org.ruby.client.ui.Settings.PREF_WIFI_DISABLED,!enable);
    		edit.commit();
    		if (enable) {
		...
		public static void url(final String opt) {
	        (new Thread() {
				public void run() {
					try {
				        URL url = new URL(PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.ruby.client.ui.Settings.PREF_POSURL, org.ruby.client.ui.Settings.DEFAULT_POSURL)+
				        		"?"+opt);
				        BufferedReader in;
	    ...
	    static Intent createMWIIntent() {
			...
			if (MWI_account != null)
				intent = new Intent(Intent.ACTION_CALL, Uri.parse(MWI_account.replaceFirst("sip:","ruby:")));
			else
		...
		static boolean on_vpn() {
			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_ON_VPN, org.ruby.client.ui.Settings.DEFAULT_ON_VPN);
		}
		
		static void on_vpn(boolean enable) {
    		Editor edit = PreferenceManager.getDefaultSharedPreferences(Receiver.mContext).edit();
    		
    		edit.putBoolean(org.ruby.client.ui.Settings.PREF_ON_VPN,enable);
    		edit.commit();
		}
		...
		public static boolean isFast(int i) {
        	...
        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getString(org.ruby.client.ui.Settings.PREF_USERNAME+(i!=0?i:""),"").equals("") ||
        	...
        	if (wi != null) {
	        	...
	        	if (wi.getIpAddress() != 0 && (WifiInfo.getDetailedStateOf(wi.getSupplicantState()) == DetailedState.OBTAINING_IPADDR
	        		...
	        		on_wlan = true;
	        		if (!on_vpn())
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_WLAN+(i!=0?i:""), org.ruby.client.ui.Settings.DEFAULT_WLAN);
	        		else
	        			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_VPN+(i!=0?i:""), org.ruby.client.ui.Settings.DEFAULT_VPN);  
	        	}
        	}
        	on_wlan = false;
			return isFastGSM(i);
		...
		static boolean isFastGSM(int i) {
        	...
        	if (tm.getNetworkType() >= TelephonyManager.NETWORK_TYPE_UMTS)
        		return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_3G+(i!=0?i:""), org.ruby.client.ui.Settings.DEFAULT_3G);
        	if (tm.getNetworkType() == TelephonyManager.NETWORK_TYPE_EDGE)
       			return PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_EDGE+(i!=0?i:""), org.ruby.client.ui.Settings.DEFAULT_EDGE);
        	return false;
		}
		...
		public void onReceive(Context context, Intent intent) {
	        ...
	        if (intentAction.equals(Intent.ACTION_SCREEN_OFF)) {
	        	...
	        	if (RubyEngine.pwl != null)
	        		for (PowerManager.WakeLock pwl : RubyEngine.pwl)
	        ...
	        if (intentAction.equals(WifiManager.SCAN_RESULTS_AVAILABLE_ACTION)) {
	        	if (PreferenceManager.getDefaultSharedPreferences(mContext).getBoolean(org.ruby.client.ui.Settings.PREF_SELECTWIFI, org.ruby.client.ui.Settings.DEFAULT_SELECTWIFI)) {
		        	WifiManager wm = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
		        	WifiInfo wi = wm.getConnectionInfo();
